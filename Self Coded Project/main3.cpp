...
public:
    ...
    {
        string filename = "solution_" + to_string(studentID) + "_" + to_string(classID) + "_" + to_string(examType) + ".txt";
    ...
    pair<int, int> loadStudentPerformance(int studentID, int classID, int examType)
    {
        string filename = "solution_" + to_string(studentID) + "_" + to_string(classID) + "_" + to_string(examType) + ".txt";
        ...
        {
            string line;
            getline(file, line); // Skip Student ID line
            getline(file, line); // Skip Class ID line
            getline(file, line); // Skip Exam Type line
            getline(file, line); // Skip Student Answers line
            getline(file, line); // Correct Answers line
            vector<int> correctAnswers;
            stringstream ss(line.substr(16));
            int correctOption;
            while (ss >> correctOption)
            {
                correctAnswers.push_back(correctOption);
            }
            file.clear();
            ...
            getline(file, line); // Skip Class ID line
            getline(file, line); // Skip Exam Type line
            getline(file, line); // Student Answers line
            vector<int> studentAnswers;
            stringstream ss2(line.substr(16));
            int studentAnswer;
            while (ss2 >> studentAnswer)
            {
...
int findClassIndexByID(int classID)
{
...
int findTeacherIndexByUsername(const string &username)
{
...
int findStudentIndexByUsername(const string &username)
{
...
void resultMenu(int studentIndex)
{
    int classID = students[studentIndex].classID;
    cout << "Select exam type to view result:\n1. Quiz\n2. Midterm\n3. Final\n";
    int examType;
    while (!(cin >> examType) || examType < 1 || examType > 3)
    {
    ...
    int score, total; // Declare score and total here
    std::tie(score, total) = exam.loadStudentPerformance(students[studentIndex].id, classID, examType);
    // or use auto score_total = exam.loadStudentPerformance(students[studentIndex].id, classID, examType); and then score=score_total.first; total=score_total.second;

    if (total == 0)
    {
        cout << "No exam taken for the selected exam type." << endl;
        return;
    }
